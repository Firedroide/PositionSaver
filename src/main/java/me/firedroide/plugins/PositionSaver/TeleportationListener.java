...
public class TeleportationListener implements Listener {
	...
	public void onPlayerTeleport(PlayerTeleportEvent e) {
		...
		if (e.getCause().equals(TeleportCause.NETHER_PORTAL) || e.getCause().equals(TeleportCause.END_PORTAL)) return;

		ps.setLastPosition(e.getPlayer(), e.getFrom());

		if (e.getCause().equals(TeleportCause.COMMAND) && !ps.getChangeTeleportCommands()) return;
	...
	private boolean isIgnored(Player p, String group) {
		boolean i = p.isPermissionSet("positionsaver.ignored") && p.hasPermission("positionsaver.ignored");
